...
평가 항목은 넥슨이 공개적으로 안내한 AI 활용 역량평가의 고수준 설명인 AI 이해도, 문제 접근 방식과 구조적 사고, 프롬프트 작성 과정, 최종 결과물의 완성도를 참고합니다. 공개된 공식 세부 채점표나 가중치가 있다고 주장하지 말고, 아래 항목을 학습용 자체 피드백으로 분명히 구분합니다.

- **긍정적:** 세션에서 확인된 구체적인 행동과 근거
- **부정적 / 보완점:** 놓친 조건, 모호했던 요청, 검증이 부족했던 지점 등 관찰된 근거. 부정적 항목이 없으면 억지로 만들지 말고 확인되지 않았다고 씁니다.
- **개선 방안:** 다음 작업에서 실행할 수 있는 구체적인 행동

사용자의 인성이나 일반 능력을 평가하지 말고 해당 세션에서 관찰된 AI 협업 행동만 평가합니다. 테스트나 측정 결과를 확인하지 않았다면 확인한 것처럼 서술하지 않습니다. 매번 오늘 세션에서 얻은 근거를 사용하며 이전 피드백을 그대로 반복하지 않습니다.

참고한 공개 안내:

- [서울신문: 넥슨 게임 프로그래머 채용의 AI 활용 역량평가 소개](https://www.seoul.co.kr/news/economy/2026/08/27/20260827500045)
- [전자신문: 넥토리얼 AI 활용 역량평가 도입 및 평가 방향](https://www.etnews.com/20260825000249)

## 커밋 메시지 형식

기능 작업 커밋은 Sprint 구현 순번을 사용하며, 커밋 횟수나 한 구현 안의 하위 작업 수로 번호를 늘리지 않습니다. 현재 순서는 1) Profiler / Stress Test, 2-1) Sprite Batch, 2-2) Offscreen Render Target / Post Processing 순입니다.

- 제목은 `AI Sprint <구현 순번> <기능 요약>` 형식으로 작성합니다. 따라서 Sprite Batch는 `AI Sprint 2-1 ...`, Offscreen Render Target / Post Processing은 `AI Sprint 2-2 ...`로 표기합니다. 이 저장소의 기존 제목처럼 한국어로 변경 내용을 구체적으로 요약하며, 사용자가 별도 형식을 지정하지 않는 한 Conventional Commits 접두어(`feat:`, `fix:` 등)는 붙이지 않습니다.
