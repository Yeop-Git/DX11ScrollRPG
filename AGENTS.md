...
## 9. README 구성과 문장 종결 규약

- 직접 구현한 기반과 AI Sprint의 확장 구현을 구분합니다.
- Thread Pool, Job Queue, JSON 데이터 분리·비동기 파일 로딩은 AI Sprint 하위 항목으로 배치합니다. AI Sprint 개요와 목차에도 해당 범위를 반영합니다.
- 기능 설명은 기존의 개발 배경·구현 내용·검증 결과·한계 및 개선점 형식을 따릅니다.
- README 문장은 기존의 간결한 명사형·동사형 종결로 통일합니다. 예: `설정 분리.`, `Worker 종료 처리.`, `검증 통과`, `추가 확인 필요`, `핫 리로드 미지원`.
- `구현했습니다`, `처리합니다`, `확인할 수 있습니다` 같은 존댓말·대화체는 사용하지 않습니다. 각각 `구현`, `처리`, `확인 가능`처럼 정리합니다.
- 중복 설명, 불필요한 수식어와 작업 과정의 서술을 줄이고 목적·구조·결과·한계를 중심으로 작성합니다.
- 구현 완료와 후속 계획, 자동 검증과 사용자 육안 확인을 구분하며 코드·측정값·제약은 보존합니다.
- AI Sprint 비교 표에는 변동이 있으며 효과·한계 설명에 필요한 지표만 남깁니다. 변동 없는 열과 중복 지표는 제외하고 원본 측정 기록은 보존합니다.
- Render Queue 비교 표는 `Render CPU`로 표기하고 실제 Render 측정값만 사용합니다. Queue 하위 구간을 병기하거나 합산하지 않습니다. 단위와 비교 방향은 열 제목에 명시합니다.
- 이 문체 규약은 README에 적용합니다. AGENTS.md의 기존 지시문 문체는 유지합니다.
...
- `v0.1.0`은 README의 **기본: 직접 작성한 코드** 1~5번 작업을 포함한 릴리스입니다. AI Sprint는 이 버전 이후의 확장 작업이며 해당 배포본에 포함되지 않습니다.
- 릴리스 태그는 `v<Major>.<Minor>.<Patch>` 형식을 사용합니다. 검증된 배포 단위를 나타내며, 개별 작업마다 자동으로 버전을 올리지 않습니다.
- `origin#<번호>`는 직접 구현한 작업 이력, `ai-Sprint#<일차>-<작업 순번>`은 AI Sprint 작업 이력입니다. 두 태그를 릴리스 버전이나 배포 완료 표시로 사용하지 않습니다.

### 버전 증가 기준

| 구분 | 기준 |
| --- | --- |
| Major | 정식 1.0 전환 또는 기존 사용·데이터 호환성을 깨는 주요 변경 |
| Minor | 기능 묶음 추가와 개발 단계 확장. 0.x 개발 단계의 호환성 변경은 영향 범위를 릴리스 노트에 명시 |
| Patch | 기존 기능의 오류 수정과 동작을 보존하는 소규모 보완 |

- 상위 버전을 올리면 하위 번호는 0으로 초기화합니다. 문서·주석 수정만으로는 릴리스 버전을 올리지 않습니다.
- 다음 버전은 실제 배포 범위와 검증 결과를 기준으로 정합니다. 계획 중인 버전을 README에 배포 완료로 표시하지 않습니다.

### 릴리스 기록과 보존

- 릴리스 태그는 배포 대상 커밋을 가리키도록 생성하고, 실행 파일·필수 Assets·Shaders가 해당 커밋과 일치하는지 확인합니다.
- 릴리스 노트에 포함 기능, 이전 버전과의 차이, 빌드·실행 검증 결과, 호환성 변경, 알려진 한계를 기록합니다.
- README의 다운로드 링크·버전·포함 기능을 실제 릴리스와 맞춥니다. 현재 개발 브랜치의 기능과 배포본의 기능을 구분합니다.
- 기존 릴리스·작업 태그는 재지정하거나 덮어쓰지 않습니다. 배포 후 수정은 새 커밋과 다음 릴리스로 기록합니다.
- 로컬 커밋·태그 생성과 원격 push·릴리스 게시는 구분합니다. 원격 반영과 게시는 사용자 요청 범위에 따라 수행합니다.
