...
// Worker가 읽은 경로 목록을 받아 메인 스레드에서 디코딩과 D3D 리소스 생성을 수행한다.
bool ResourceManager::Initialize(ID3D11Device* device, const std::unordered_map<SpriteId, std::string>& textures)
{
	for (const auto& [id, path] : textures)
	{
		if (!LoadTextrue(device, id, path.c_str()))
		{
			Clear();
